    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <form onSubmit={handleSubmit} className="lg:col-span-5 bg-mining-card border border-mining-border p-6 rounded-2xl flex flex-col gap-5">
        <div>
          ...
          <p className="text-xs text-gray-400">Define drill pattern geometry and explosive charge distribution</p>
        </div>

        {/* Identification */}
        <div className="border-t border-mining-border pt-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-gray-400 font-semibold">Blast Plan ID</label>
            <input
              type="text"
              name="blast_id"
              value={form.blast_id}
              ...
              className="bg-mining-dark border border-mining-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-mining-accent"
            />
          </div>
        </div>
        ...
        <div className="border-t border-mining-border pt-4">
          <div className="flex justify-between items-center mb-3">
            ...
            <button
              ...
              className="text-[10px] bg-mining-accent/20 hover:bg-mining-accent/40 border border-mining-accent/40 text-mining-gold px-2 py-0.5 rounded flex items-center gap-1 transition-colors"
            >
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Bench Height (m)</label>
              <input
                type="number" step="0.1" name="bench_height"
                value={form.bench_height} onChange={handleInputChange} required
                className="bg-mining-dark border border-mining-border rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none"
              />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Hole Diameter (mm)</label>
              <input
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Burden (m)</label>
              <input
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Spacing (m)</label>
              <input
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Subdrill (m)</label>
              <input
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Stemming Height (m)</label>
              <input
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs text-gray-400">Drillhole Layout Layout</label>
            <select
              name="hole_layout"
        ...
        <div className="border-t border-mining-border pt-4">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Explosive Type</label>
              <select
                name="explosive_type"
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Qty Per Hole (kg)</label>
              <input
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Primer Position</label>
              <select
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Detonation Delay (ms)</label>
              <input
          ...
          <div className="flex justify-between items-center bg-mining-dark border border-mining-border p-3 rounded-lg">
            <span className="text-xs text-gray-300">Deck Charging Configuration</span>
            <input
              type="checkbox"
              name="deck_charging"
      ...
      <div className="lg:col-span-7 flex flex-col gap-6">
        ...
        {optSuggestions && (
          <div className="bg-gradient-to-r from-gray-900 to-mining-card border border-mining-gold/30 p-4 rounded-2xl flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
              <div className="bg-mining-dark/60 p-2.5 rounded-lg border border-mining-border">
                <span className="text-[9px] text-gray-400 block uppercase">Optimal Burden</span>
                <span className="text-sm font-bold text-white">{optSuggestions.opt_burden} m</span>
              </div>
              <div className="bg-mining-dark/60 p-2.5 rounded-lg border border-mining-border">
                <span className="text-[9px] text-gray-400 block uppercase">Optimal Spacing</span>
                <span className="text-sm font-bold text-white">{optSuggestions.opt_spacing} m</span>
              </div>
              <div className="bg-mining-dark/60 p-2.5 rounded-lg border border-mining-border">
                <span className="text-[9px] text-gray-400 block uppercase">Optimal Hole Depth</span>
                <span className="text-sm font-bold text-white">{optSuggestions.opt_hole_depth} m</span>
              </div>
              <div className="bg-mining-dark/60 p-2.5 rounded-lg border border-mining-border">
                <span className="text-[9px] text-gray-400 block uppercase">Powder Factor Target</span>
                <span className="text-sm font-bold text-white">{optSuggestions.opt_powder_factor} kg/m³</span>
              </div>
            </div>
        ...
        {!result ? (
          <div className="bg-mining-card border border-mining-border p-6 rounded-2xl flex flex-col items-center justify-center text-center h-full min-h-[350px]">
            <Zap size={48} className="text-gray-600 mb-3 animate-pulse" />
            <h3 className="text-white font-semibold text-sm">Awaiting Blast Plan Submission</h3>
            <p className="text-xs text-gray-400 max-w-[280px] mt-1">Submit pattern and charges to view 2D layouts, delay animation waveforms, and Rosin-Rammler fragmentation curves</p>
          </div>
        ) : (
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-mining-card border border-mining-border p-6 rounded-2xl flex flex-col gap-4">
              <div>
                <h3 className="text-sm font-bold text-white uppercase tracking-wider text-mining-accent">3. Blast Simulation Output</h3>
                <p className="text-xs text-gray-400">Calculated impact metrics at 150m monitoring distance</p>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
                <div className="bg-mining-dark/60 p-3 rounded-xl border border-mining-border flex flex-col justify-between">
                  <span className="text-[9px] text-gray-400 block uppercase font-bold">Ground PPV</span>
                  <span className={`text-lg font-black ${result.ground_vibration > 15 ? 'text-red-400' : 'text-green-400'}`}>
                    {result.ground_vibration} mm/s
                  </span>
                  <span className="text-[8px] text-gray-500 block mt-1">Safe Limit: 15</span>
                </div>
                <div className="bg-mining-dark/60 p-3 rounded-xl border border-mining-border flex flex-col justify-between">
                  <span className="text-[9px] text-gray-400 block uppercase font-bold">Air Blast Noise</span>
                  <span className={`text-lg font-black ${result.air_blast > 120 ? 'text-orange-400' : 'text-green-400'}`}>
                    {result.air_blast} dB
                  </span>
                  <span className="text-[8px] text-gray-500 block mt-1">Safe Limit: 120</span>
                </div>
                <div className="bg-mining-dark/60 p-3 rounded-xl border border-mining-border flex flex-col justify-between">
                  <span className="text-[9px] text-gray-400 block uppercase font-bold">Throw Distance</span>
                  <span className="text-lg font-black text-mining-gold">
                    {result.throw_distance} m
                  </span>
                  <span className="text-[8px] text-gray-500 block mt-1">Ideal: 15 - 35m</span>
                </div>
                <div className="bg-mining-dark/60 p-3 rounded-xl border border-mining-border flex flex-col justify-between">
                  <span className="text-[9px] text-gray-400 block uppercase font-bold">Flyrock Risk</span>
                  ...
                  <span className="text-[8px] text-gray-500 block mt-1">Safety zone active</span>
                </div>
              </div>

              {/* Warning flags if critical */}
              {(result.ground_vibration > 15 || result.air_blast > 120 || result.flyrock_risk === 'HIGH') && (
                <div className="bg-red-950/20 border border-red-800 text-red-400 p-3 rounded-lg text-xs flex items-start gap-2">
